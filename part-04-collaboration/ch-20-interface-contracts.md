...
> **前置閱讀**:[第 18 章｜Pull Request 的拆分與描述](./ch-18-pull-request.md) ⸺ 理解如何把改動說清楚,是協調介面變更的前提
> **下游章節**:[第 21 章｜CI/CD 流水線設計](../part-05-delivery/ch-21-cicd.md) ⸺ 契約測試要能自動跑,才能在流水線裡守住

---

## 20.1 共感現場:那個「我以為你不會動它」的週一早上
...
這三個場景的共同根源是一樣的:「API 的邊界存在,但責任的邊界沒有被說清楚。」也就是說,當責任邊界沒有被說清楚時,雙方都只能陷入猜測——提供方猜「這個改動應該不會有人在意吧」,消費方猜「這個欄位應該一直都會在吧」。猜測不是協作,它只是把風險延後到週一早上。
...
## 20.3 一起做判斷:建立介面契約的四件事

那麼,一個團隊要怎麼開始把「有 API」變成「有契約」?

這件事不複雜,但需要把幾個步驟變成習慣,而且這幾個步驟是逐步遞進的:先定義穩定性等級(19.3.1),這樣才知道改動時要多謹慎;有了穩定性的判斷,才談得上具體的版本相容策略(19.3.2);策略定了,才有東西可以寫成契約測試自動驗證(19.3.3);最後再從服務依賴的全景,看這些個別契約要怎麼整合成整體的相依方向(19.3.4)。

### 20.3.1 先把「穩定 vs 實驗性」說清楚
...
有了這個區分,帳務團隊改 `plan_name` 這件事就會很不一樣。如果那支 API 被標記為「穩定介面」,改動之前就需要跟消費方協調——具體怎麼協調,我們在下一小節會看。如果是「實驗性介面」,那核心平台團隊在使用時就要知道它可能變動,要為此設計防護——例如用 fallback 邏輯應對欄位消失(讀不到 `plan_name` 就退回一個預設值,而不是直接拋例外),或用 feature flag 控制對新欄位的依賴,這樣即使 API 變動,也不會讓整個計費流程跟著爆開。
...
### 20.3.3 用契約測試(Contract Testing)把協議落地

光有口頭協議是不夠的,因為時間一長,口頭協議就會模糊。把契約變成可執行的測試,才能讓它在每次部署時都自動被驗證。

契約測試(Contract Testing)的概念很直接:消費方定義「它期待 API 長什麼樣」——例如在 FlowBill 的案例中,核心平台團隊會明確寫下「我期待 `GET /v1/subscription/{id}` 回傳的 JSON 裡有一個叫 `plan_name` 的欄位,型別是 string,而且這個欄位名稱不會變」。提供方再執行這份期待,確認自己有沒有破壞它。市面上的工具(例如 Pact,pact.io)把這份期待自動化成一份 pact 檔案:消費方的測試產出這份檔案,描述的就是上面那類「欄位存在、型別是什麼、名稱不會變」的具體斷言,提供方的 CI 流水線在每次發布前都拿這份檔案來跑驗證。
...
### 20.5.1 範例:FlowBill 訂閱 API 的介面契約書
...
- **前一章**:[第 19 章｜結對/群體程式設計的時機](./ch-19-pairing.md) ⸺ 介面設計的早期討論,結對是個好時機
- **下一章**:[第 21 章｜CI/CD 流水線設計](../part-05-delivery/ch-21-cicd.md) ⸺ 契約測試要能自動跑,流水線是它的家
- **強連結**:[第 13 章｜契約測試與整合測試](../part-03-testing/ch-13-contract-integration.md) ⸺ Pact 的具體寫法與測試雙方的角色
- **強連結**:[第 16 章｜與 CI 整合的測試流水線](../part-03-testing/ch-16-ci-testing.md) ⸺ pact verify 要在哪個階段跑
