...
> **前置閱讀**:[第 26 章｜可觀測性落地](./ch-26-observability.md)、[第 27 章｜從告警到根因](./ch-27-alert-to-rootcause.md)
> **下游章節**:[第 29 章｜On-call 與事故處理](./ch-29-on-call.md)、[第 46 章｜判斷力的養成](../part-09-synthesis/ch-46-cultivating-judgment.md)

## 28.1 共感現場:「我試了三天,還是不知道為什麼」
...
小璇遇到的,正是這個情形。她的日常工作是在 Spring Boot 這一層——寫 service、寫 repository、看 application log。PostgreSQL 查詢計畫、Kafka 的 broker 配置、OS 層的 I/O 排程器——這些都在她的日常視野之外。

這意味著什麼?當問題指向那些層時,她既沒有觀測工具來確認症狀出在哪裡,也沒有該層的因果模型來快速生成假設——她得從零開始摸索。沒有觀測工具,就看不到現象;看不到現象,就提不出「如果是這個原因,應該會看到什麼」這種可驗證的假設;提不出假設,就只能一個一個去試。所以當問題跨越了那條線,她的搜尋空間就從「哪一行程式碼」擴展成了「整條 stack 上的任何一點」,而那個空間大到沒有窮舉的可能。

順著這個道理,我們就能看出:問題不在於小璇不夠努力,而在於**她的搜尋策略是線性的——一個可能性、一個可能性地試——但問題空間是指數的**。唯一能讓搜尋收斂的,是先有一個假設,把搜尋空間縮小到一層,然後在那一層求證。

那麼,什麼樣的假設才能真正做到這一點?這就帶出了本章最核心的一個觀念:**大膽假設、小心求證**。

## 28.3 一起做判斷:有地圖的除錯 vs 沒有地圖的試誤
...
沒有因果模型的人用 AI 的方式往往是:**「我的 API 超時,幫我看看這段程式碼有什麼問題?」** 然後把 AI 給的建議一個個貼上去試。這樣做看起來在積極探索,但實際上仍然是逐個試誤——只是每輪試誤的速度更快了。這就是 **vibe-debugging**:AI 建議一個可能性,你試一輪;AI 再給一個,你再試一輪。搜尋空間沒有縮小,只是在用更快的工具在裡面繞圈。

也就是說,**AI 不會補上缺失的穿透力,它放大「有無因果地圖」的差距,而不是彌平它**。手上有地圖的人,AI 讓他們下降得更快;沒有地圖的人,AI 讓他們繞圈繞得更勤快。

### 28.3.4 穿透的前提:每一層都要先有觀測孔
...
這個問題在小璇身上也出現了——解決 autovacuum 問題之後,她知道了 PostgreSQL 17 有 autovacuum 這個行為,也知道了怎麼用 `pg_stat_activity` 觀測。但這份知識沒有記錄在團隊的 wiki、runbook 或文件裡。三個月後換別人接手,同樣的問題從頭再走一遍。
...
### 28.5.1 範例:Finyuan 清算超時事件

讓我們回到小璇那個偶發清算超時的問題。如果她一開始就用這張地圖來組織自己的除錯思路,這個問題大概可以在第一個工作天的下午就收斂。

**為什麼「觀察到什麼」這一欄很關鍵**:「平均正常但 p99 高」是非常有價值的信號——它幾乎直接排除了「查詢邏輯本身低效」的可能性,因為那會讓平均也高。指向的是「偶爾觸發的競爭或等待」。這個判斷,就是下面整張地圖能收斂的起點。
...
上面這份紀錄裡有兩個數字值得多說一句。第一個是「驗證結果」欄裡的 `n_dead_tup = 1,240,000`——看到 dead tuple 數量超過百萬,就能估算 autovacuum 需要清理的工作量,也能估算它對 IO 的影響程度,這個數字讓假設從「可能是 autovacuum」變成「幾乎確定是 autovacuum」。第二個是輪次 2 裡的參數改動——記錄「改了什麼」的同時也記錄「為什麼改成這個值」,這樣三個月後接手的人才看得懂這些參數不是隨機調出來的「魔法數字」,而是有因果依據的決定。
...
- **上一章**:[第 27 章｜從告警到根因](./ch-27-alert-to-rootcause.md) ⸺ 本章的前半段:如何從 alert 開始收斂
- **下一章**:[第 29 章｜On-call 與事故處理](./ch-29-on-call.md) ⸺ 把穿透力用在生產事故的即時決策
- **強連結**:[第 26 章｜可觀測性落地](./ch-26-observability.md) ⸺ 穿透需要工具;metrics/trace/log 是你的觀測眼
- **強連結**:[第 46 章｜判斷力的養成](../part-09-synthesis/ch-46-cultivating-judgment.md) ⸺ 「被逼著下降的肌肉」如何在 AI 時代刻意培養
- **強連結**:[第 37 章｜AI 輔助編碼的工作流重塑](../part-08-ai-era/ch-37-ai-assisted-coding.md) ⸺ vibe-debugging 反模式的完整討論
