...
那種「我已經很努力查了,為什麼進度還是一片亂」的心境,很多工程師都懂。面對一個陌生的故障現場,你知道事情很嚴重,你也想快點找到答案,但不知道「接下來一步」是什麼——這種卡住的感覺,比技術本身更讓人耗盡。
...
這裡有個認知科學的背景值得了解。大腦在高壓下處理並行任務時,並不是真正「同時進行」,而是快速在任務之間切換——心理學上叫「任務切換(Task Switching)」。每一次切換都有成本:研究估計,每次從一個思維脈絡切換到另一個,需要 15 到 20 分鐘才能重新「全力投入」。阿銘那晚的情況,就是在 debug(需要深度專注)、回應 Slack(需要切換情境)、評估是否升級(需要做判斷)之間不斷切換。每一次切換都讓除錯效率大幅下降。

**二、資訊真空。** 頻道裡沒有狀態更新,每個人都不知道現在進展到哪,於是大家都開始發問。

沒有更新為什麼會導致「噪音」?因為人在不確定的情況下會主動尋找資訊。如果找不到,就會用「發問」來填補空白。每一個發問本身都是善意的,但累積起來就成了噪音——頻道裡有十幾則「有進展嗎」「需要我幫什麼」「現在狀況怎樣」,唯一在查問題的阿銘每一則都要花時間判斷「我需要回這個嗎」。這個判斷本身,就是更多的認知負荷。
...
一個好用的判斷原則是:**如果你在猶豫「這算不算 Sev1」,那八成已經是了。** Sev1 的核心特徵不是指標有沒有跌破某條線,而是「這件事有沒有在主動傷害用戶」。成功率 85% 意味著每 7 筆轉帳就有 1 筆失敗——對一個金融服務來說,這已經是主動傷害,不是「降級」。
...
值班工程師有時候會想「再查一下再說,叫人好像大題小作」。這個心態很理解——沒有人想給別人添麻煩,特別是深夜叫醒別人這件事。

但為什麼這樣反而更有問題?根本原因在於 Sev1 的本質是「一個人的注意力不夠用的情境」。當你同時要除錯、要管頻道、要做決定,三件事都做得不夠好。再加上壓力下的認知負荷,你做的每一個判斷品質都在下降。把時間花在「要不要叫人」的猶豫上,其實是在用最貴的資源(你在壓力下的認知能力)做一個本來應該提前做好的決定。
...
### 28.5.2 範例:PingPay 的 Sev1 Runbook
...
你看到這份範例和阿銘那晚的差別了嗎?差別不是工程師更聰明,而是**流程設計幫他在壓力下維持了正確的順序**。Step 2 裡的那行 `openssl` 指令,在阿銘不知道去哪裡找的時候,已經等在他面前了。

這份 Runbook 每一欄背後的考量,呼應的就是 §28.2 分析的三個問題:決策空白(靠嚴重度分級和 IC 流程解決)、資訊真空(靠固定更新節奏解決)、止血/根因混淆(靠「止血手段」區塊和「服務恢復後」清單的先後順序解決)。
