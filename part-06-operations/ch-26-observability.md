...
> **前置閱讀**:[第 25 章｜資料庫遷移與零停機變更](../part-05-delivery/ch-25-db-migration.md)
> **下游章節**:[第 27 章｜從告警到根因:生產環境除錯](./ch-27-alert-to-rootcause.md)

## 26.1 共感現場:打開 Kibana,什麼都沒搜到
...
這裡的問題不只是缺一條線索,而是系統本身沒有被設計成「能讓外部觀察者推斷內部狀態」。一個好設計的系統,應該讓任何一份請求都自帶完整的上下文——不是靠 on-call 工程師去猜。
...
在工具的選擇上,各語言都有成熟的選項:Node.js 生態系的 `pino`(v8.x)是效能最好且預設輸出 JSON 的選項;Python 有 `structlog`(v23.x);Java 的話 `logback`(v1.4.x)配合 `logstash-logback-encoder` 是常見組合。這些工具之間的差異其實沒有「整個 service 的 log 格式一致」這件事重要——換句話說,pino 和 structlog 誰效能更好,沒有「log 聚合系統能統一解析」重要。
...
業務層 Metric 的核心是「用戶看得到的成敗指標」。支付系統看的是支付成功率,電商系統看的是訂單轉換率,社交應用可能是訊息送達率或登入成功率,基礎設施平台可能是資源配置成功率——不管領域是什麼,它都是最直接反映「系統對用戶有沒有價值」的那個數字,所以永遠值得優先建。
...
但這裡有個容易遺漏的細節:自動埋點雖然覆蓋了 HTTP 呼叫和資料庫查詢,卻看不到應用邏輯本身的耗時。很多時候,整筆請求最慢的地方不是外部呼叫,而是應用代碼裡的業務計算——風控查詢、費用計算、資料轉換。手動埋點的場景正是業務邏輯層的關鍵路徑,例如「計算動態手續費率」或「查詢風控規則」這類業務上有意義、但框架不知道怎麼自動識別的操作。在這些地方加上自訂 span,讓 Trace 不只告訴你「哪個 HTTP 呼叫慢」,還告訴你「業務邏輯的哪個決策點花了多少時間」。
...
> **修正方向**:為 log level 的語意建立團隊共識(見 §26.3.1 的對照表),把共識寫進 ADR,比工具配置更重要。拿一個已有的 codebase 試試:搜 `level:ERROR`,看看第一頁裡有幾條是真的讓你需要馬上處理的。如果不到一半,就是 level 語意需要重整的訊號。
...
這些錯誤的共同根源,就是缺少一個系統性的驗收標準。所以很多工程師會在「好像埋得差不多了」的感覺下上線,結果才發現其實漏了一半。下一節我要分享的,是一張能逐項確認的清單——它會強迫你逐個檢查 Log / Metric / Trace 層有沒有真的埋完整。

## 26.5 帶得走的工具 ⸺ 一頁式「可觀測性埋點清單」
...
- **下一章**:[第 27 章｜從告警到根因:生產環境除錯](./ch-27-alert-to-rootcause.md) ⸺ 可觀測性是燃料,這章教你怎麼開車
- **前章**:[第 25 章｜資料庫遷移與零停機變更](../part-05-delivery/ch-25-db-migration.md) ⸺ 遷移過程中的可觀測性同樣重要
- **強連結**:[第 28 章｜向下穿透抽象層](./ch-28-penetrating-abstraction.md) ⸺ 當 Trace 追到底層,你需要下潛的能力
- **強連結**:[第 29 章｜On-call 與事故處理](./ch-29-on-call.md) ⸺ 本章建的基礎設施,在 on-call 時發揮作用
