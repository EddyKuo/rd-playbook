...
# 第 27 章｜從告警到根因:生產環境除錯
## ⸺ 大膽假設、小心求證:讓每一步都縮小搜尋範圍

> **前置閱讀**:[第 26 章｜可觀測性落地](./ch-26-observability.md) ⸺ log/metric/trace 是本章的彈藥;沒有它們,除錯只能靠猜
> **下游章節**:[第 28 章｜向下穿透抽象層](./ch-28-penetrating-abstraction.md) ⸺ 當根因跨越了你熟悉的那個層,就需要下鑽

## 27.1 共感現場:那個同時響起三支電話的清晨

你可能也遇過這樣的早晨。

凌晨四點半,小涵的手機同時跳出三條 PagerDuty 告警:支付成功率下滑、p99 延遲飆到 8 秒、交易失敗數每分鐘超標。她坐起來,第一個念頭幾乎是所有人都有過的那種——先重啟服務看看會不會好。

小涵後來回憶起那個清晨(下面這個故事基於一個虛構的支付平台 ClearPay,但那種手足無措的感覺,很多工程師都不陌生):「那時候腦袋空空的,只覺得要做點什麼,不能只是呆坐著。」
...
## 27.3 一起做判斷:假設驅動除錯的四個步驟

讓我帶你走一遍這個方法,順著小涵那天早晨的案例來看。

小涵所在的公司 **ClearPay**,系統架構大致是:前端 API 層 → 支付業務服務 → 外部清算閘道器(第三方)+ 自有資料庫。那天凌晨的症狀是支付成功率驟降、延遲飆升。
...
每個假設對應的驗證方式不同。優先驗「最快能取證、對正常流量影響最小」的那個——這是兩個維度,值得分開看。比如查外部服務的 status page,只需要三十秒、不改任何設定,取證速度快又零風險;重啟服務雖然也快,卻會讓正在進行中的交易中斷,等於是拿正常流量去冒險。查資料庫的慢查詢日誌通常也是又快又安全的選擇;相對地,調整機器資源去試探容量問題,則會牽涉成本與風險,適合放到比較後面才驗。
...
確認了「問題在哪個層」之後,還需要問「具體是什麼」——定位到層只是把搜尋空間從整個系統縮小到一個子系統,真正的根因往往還需要再往下一層追。這個步驟其實包含三個緊密相連的子動作:**驗證**假設是否成立、**下鑽**到更具體的細節、最後**確認根因**,所以下面的整理表會把它拆成兩列來看,但它們本質上是同一段推理的延伸,不是新的階段。

確認了「外部閘道器有問題」之後,下一個問題不是「怎麼修」,而是「這是他們那邊的問題,還是我們給他們的請求格式有問題?」繼續假設 → 驗證。

查閘道器的 status page:那個時間點他們確實有一次計畫外維護,影響了特定地區的節點。根因確認:外部服務降級導致 timeout 堆積。應對方式隨之清晰:在閘道器回應緩慢時啟用降級流程、設定 circuit breaker 斷路。

把這個流程整理成一個可以複用的結構(下表的④和⑤,對應的正是步驟四裡「驗證、下鑽、確認根因」這三個子動作):
...
> **修正方向**:先完整列假設清單,然後排出優先序——哪個假設如果成立最能解釋所有症狀?先驗最高可能性的那個。驗完之前,不要跳到下一個。如果是多人協作,分工也要順著這個優先序來,而不是各查各的:A 負責驗證「是否外部依賴慢」——查對方 status page、查 outbound 延遲 metric;B 負責驗證「是否自己的 DB 或業務層慢」——查 query log、查資源指標。在 A 有結果之前,B 可以先待命而不是搶著開查,免得兩個人在同一條路上重複勞動。只有當兩個假設彼此不互斥、可以同時驗證而不互相干擾時,才適合真正平行進行。
...
═══════════════════════════════════════════
① 邊界觀察 // 時間點是排除假設的第一條線索,先把「卡住感」變成結構化事實
─────────────────────────────────────────
...
### 27.5.1 範例:ClearPay 2026-03-15 支付成功率事件
...
你看,小涵最後找到的根因,不是一個很難發現的東西——閘道器的 metric 在 Grafana 上都有,status page 也是公開的。真正讓那個早晨變得很長的,是沒有一個結構幫她知道「先看什麼、看完之後問什麼」。小涵後來復盤時說,如果那天手上就有這張卡,那段路不會是摸索式亂試,而是會變成有邏輯的推進——每填一欄,就少走一段冤枉路,省下來的時間她估計不只一半。下次你遇到同樣的凌晨四點,它就是你的那張地圖。
...
- **上一章**:[第 26 章｜可觀測性落地](./ch-26-observability.md) ⸺ log/metric/trace 是本章除錯的彈藥來源
- **下一章**:[第 28 章｜向下穿透抽象層](./ch-28-penetrating-abstraction.md) ⸺ 當根因跨越了熟悉的層,就需要「大膽假設、小心求證」的下鑽技藝
- **強連結**:[第 29 章｜On-call 與事故處理](./ch-29-on-call.md) ⸺ 本章是個人除錯思路,第 29 章是事故時的團隊協作協定
