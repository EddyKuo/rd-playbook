...
> **前置閱讀**:[第 16 章｜與 CI 整合的測試流水線](../part-03-testing/ch-16-ci-testing.md)、[第 18 章｜Pull Request 的拆分與描述](../part-04-collaboration/ch-18-pull-request.md)
> **下游章節**:[第 22 章｜Feature Flag 與漸進式發布](./ch-22-feature-flags.md)、[第 23 章｜藍綠/金絲雀部署](./ch-23-blue-green-canary.md)

## 21.1 共感現場:那個每次都在「希望不要壞」的部署時刻
...
為了讓這個問題永遠不再發生,團隊需要做的第一件事就是:把這些原本只住在個人頭腦裡的經驗,變成任何人都能執行、結果都可預測的流程。順著這個道理,我們就能看到 CI/CD(持續整合/持續交付,Continuous Integration / Continuous Delivery)的本質不是工具,而是一個承諾——對整個團隊承諾:這條路,我們走過、測過、記錄過,下一次走也會一樣安全。

## 21.2 真正的問題:可重現性不是「剛好」,是設計出來的
...
## 21.3 一起做判斷:流水線的五個階段與品質門檻

一條 CI/CD 流水線不是越長越好,也不是越短越快。它的設計原則只有一個:
**讓每個問題,盡量在距離它產生最近的那個位置被擋下來。**

理由很簡單——越晚發現,修復成本越高。在 commit 階段發現的問題,修一下就好;到了正式環境才發現,可能平均要花 2-3 小時才能完成 hotfix、測試、重新部署,期間若剛好是訂單服務故障,每小時就可能損失上千筆交易。
...
這張表想說的是:前兩段要**非常快**——如果等超過五分鐘才知道自己是否寫錯了什麼,工程師很快就會開始「跑測試的時候去做別的事」。當他們回到原本的工作時,五分鐘前改了什麼往往已經記不清楚,看到測試失敗時得重新理解上下文才能動手修。也就是說,分心會導致遺忘上下文,遺忘上下文又會讓解決問題變困難——一旦有了「跑測試就去做別的事」這個習慣,流水線的回饋價值就大打折扣。
...
- **下一章**:[第 22 章｜Feature Flag 與漸進式發布](./ch-22-feature-flags.md) ⸺ 讓部署與功能開關分離,實現漸進式上線
- **強連結**:[第 16 章｜與 CI 整合的測試流水線](../part-03-testing/ch-16-ci-testing.md) ⸺ 本章流水線的測試階段基礎
- **強連結**:[第 23 章｜藍綠/金絲雀部署](./ch-23-blue-green-canary.md) ⸺ Production Deploy 階段的策略選擇
- **強連結**:[第 24 章｜回滾與前向修復決策](./ch-24-rollback.md) ⸺ 自動回滾條件的決策細節
- **強連結**:[第 26 章｜可觀測性落地](../part-06-operations/ch-26-observability.md) ⸺ 健康檢查與錯誤率門檻的量測基礎
