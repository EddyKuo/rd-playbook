...
> **前置閱讀**:[第 24 章｜回滾與前向修復決策](./ch-24-rollback.md)
> **下游章節**:[第 26 章｜可觀測性落地](../part-06-operations/ch-26-observability.md)

## 25.1 共感現場:那個「看起來很小的欄位異動」
...
## 25.2 真正的問題:Schema 的不可逆性與部署的原子性之間的張力
...
小裴那次出事,還有第二個問題藏在裡面:他在同一次部署裡,先跑 Schema 遷移,再換應用程式版本。這代表有一段時間,舊版的應用程式拿著新版的 Schema 在服務——這不一定會炸,但一旦新舊 Schema 語義不兼容,就會悄悄地讓資料出錯,比當機還更難察覺。舉例來說,如果新 Schema 新增一個 `NOT NULL` 欄位且舊應用程式不填它,新寫入的資料就會缺這一欄;反過來,當新版應用程式已經期望這個欄位有值時,卻讀到舊資料留下的 `NULL`——兩個方向都會讓後續的邏輯判斷跑錯,而且往往不是立刻報錯,是幾天後才被發現資料兜不起來。
...
### 25.3.3 雙寫的細節:三個你一定要注意的地方

雙寫聽起來直覺,但有幾個地方容易被忽略:

**一、讀的時候先讀哪個?** 切換期間,新資料進 `status_v2` 是對的,但讀的時候要讀哪個?一個可靠的做法是:讀的時候優先讀 `status_v2`(有值的話),`NULL` 才 fallback 讀舊欄。這個邏輯確保你在歷史資料補填完成前,不會讀到空的新欄。落成 SQL 大概是這樣:

```sql
SELECT COALESCE(status_v2::text, status) AS current_status
FROM transactions
WHERE id = :id;
```

如果讀取邏輯放在應用層,道理是一樣的,只是換一種寫法:

```text
if (transaction.statusV2 != null) {
  return transaction.statusV2;
}
return transaction.status;
```

不管放在 SQL 還是應用層,原則只有一個:新欄有值就信新欄,新欄還沒補上就退回舊欄,這樣才不會在補填完成前讓使用者看到空狀態。
...
這是最常見的一個失誤。你想說:「反正我馬上就要填資料了,所以直接設 NOT NULL。」但別忘了擴張階段的前提:新版應用程式這時候還沒上線,舊版程式碼仍然是唯一在跑的版本。正因為舊程式碼不認識這個新欄位、不會主動填值,一旦它做了任何 `INSERT`,就會因為 `NOT NULL` 約束而失敗。

換句話說,部署順序直接決定了 Schema 設計要留多少空間:只要「舊程式碼仍在線上」這個時間窗口存在,新欄位就不能要求一定有值。
...
這是我們在 §25.3 討論過的那種「不兼容型別變更」——直接改會炸,所以走擴張收縮三步。
...
- **前一章**:[第 24 章｜回滾與前向修復決策](./ch-24-rollback.md) ⸺ 理解什麼時候能回滾,才知道為什麼 Schema 的不可逆性需要額外謹慎
- **下一章**:[第 26 章｜可觀測性落地](../part-06-operations/ch-26-observability.md) ⸺ 遷移期間需要監控什麼、如何確認每個步驟安全
- **強連結**:[第 21 章｜CI/CD 流水線設計](./ch-21-cicd.md) ⸺ 遷移計畫書如何整合進部署流水線的 gate 設計
- **強連結**:[第 23 章｜藍綠/金絲雀部署](./ch-23-blue-green-canary.md) ⸺ 應用程式層的零停機部署,與 Schema 層的零停機遷移如何搭配
