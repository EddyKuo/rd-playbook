...
# 第 22 章｜Feature Flag 與漸進式發布
## ⸺ 把「部署到線上」和「讓使用者看到」這兩件事,分開處理

> **前置閱讀**:[第 21 章｜CI/CD 流水線設計](./ch-21-cicd.md)
> **下游章節**:[第 23 章｜藍綠/金絲雀部署](./ch-23-blue-green-canary.md)

## 22.1 共感現場:「功能已上線,但我不敢讓人用」
...
其實這兩個問題——長期等待的焦慮,和合併時的衝突——根源是同一件事:程式碼合併的時機,被綁在了「使用者看不看得到」這個業務決定上。只要能把這兩件事拆開,問題就同時解決了一半。

第二條路,就是今天要聊的主角:**功能旗標(Feature Flag)**。它讓你可以同時做到兩件事:程式碼立刻合併到主幹(避免長期分支的衝突),但延遲對使用者開放(等業務準備好)。這樣工程節奏和業務節奏終於可以各自獨立——程式碼可以部署上線,但「使用者看不看得到這個功能」,是另一個獨立的開關。

## 22.2 真正的問題:部署和發布,其實是兩件事

我們把這件事慢慢拆開來看。

### 22.2.1 「部署 = 發布」這個假設從哪裡來
...
| 旗標種類 | 用途 | 預期存活期 | 常見錯誤 |
|---|---|---|---|
| **發布旗標(Release Flag)** | 隱藏未完成的功能,讓程式碼先上線 | 短暫,功能穩定後刪 | 容易忘記設定刪除日期 |
| **實驗旗標(Experiment Flag)** | A/B 測試、流量分流 | 中期,實驗結束後刪 | 實驗結束容易漏掉清理步驟 |
| **運維旗標(Ops Flag / Kill Switch)** | 緊急關閉有問題的功能 | 長期,需要維護 | Kill Switch 過多時容易混淆優先順序 |
| **業務旗標(Permission Flag)** | 依租戶/方案開放不同功能 | 永久(業務需要) | 界線容易模糊 |

這四種旗標的生命週期差很多:發布旗標用完就應該刪,業務旗標則可能跑幾年。把它們混在一起管理,是旗標債務最常見的根源。

值得多說一句的是「業務旗標」。它和其他三種在根本上不同:發布旗標控制的是「什麼時候」對所有人開放,業務旗標控制的是「誰能看到」。前者是漸進式發布的工具,一旦全量開放、旗標的任務就結束了;後者是永久的存取控制(Access Control)層,系統要一直用它來判斷用戶應該有什麼能力。也就是說,不要把它們混用,因為生命週期和清理策略完全不同——發布旗標建立時就該登記刪除日期,業務旗標反而需要更嚴謹的測試和文件,畢竟它會陪著產品跑很久。SaaS 產品常見的「Enterprise 方案才有這個功能」,就是典型的業務旗標場景:不需要在程式碼裡硬寫方案判斷,旗標的評估邏輯就能處理,而且這個判斷會一直存在,不會有「發布完成就刪除」的一天。
...
看表格就能看出,後端 API 層在安全性上沒有明顯短板,延遲影響也可透過本地快取控制在毫秒級,同時旗標的評估結果不會暴露給前端,規避了用戶端操控的風險。也因此,大多數場景下它是最安全的起點——你對業務邏輯的控制是完整的,也不用擔心旗標的值被用戶端看到或操控。如果日後發現 latency 有壓力,再考慮加本地快取(In-Process Cache)或下移到 CDN。
...
- **前一章**:[第 21 章｜CI/CD 流水線設計](./ch-21-cicd.md) ⸺ Feature Flag 是 CI/CD 流水線的延伸,讓「持續部署」和「受控發布」同時成立
- **下一章**:[第 23 章｜藍綠/金絲雀部署](./ch-23-blue-green-canary.md) ⸺ 漸進式發布的另一個維度:在基礎設施層做流量切換
- **強連結**:[第 24 章｜回滾與前向修復決策](./ch-24-rollback.md) ⸺ 功能旗標的 Kill Switch 是最輕量的回滾手段
- **強連結**:[第 26 章｜可觀測性落地](../part-06-operations/ch-26-observability.md) ⸺ 旗標的漸進開放需要可觀測性的配合,才能有依據地做判斷
