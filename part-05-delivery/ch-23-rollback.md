...
> **前置閱讀**:[第 22 章｜藍綠與金絲雀部署](./ch-22-blue-green-canary.md)、[第 21 章｜Feature Flag 與漸進式發布](./ch-21-feature-flags.md)
> **下游章節**:[第 24 章｜資料庫遷移與零停機變更](./ch-24-db-migration.md)、[第 26 章｜從告警到根因:生產環境除錯](../part-06-operations/ch-26-alert-to-rootcause.md)

---

## 23.1 共感現場:告警一跳,大家眼神都看向你
...
她事後說:「那時候如果憑直覺按了回滾,可能會更糟。但我那時候不知道怎麼判斷,所以我就慌了。」

這很正常。慌不是因為能力不夠,而是因為在那個時刻,腦袋裡沒有一個清楚的判斷框架可以走。這一章想給你的,就是那個框架。

---

## 23.2 真正的問題:為什麼「直覺回滾」這麼危險
...
我整理了一個三問流程,它不是要讓你在告警跳起的三秒內就想清楚所有細節,而是給你一個**可以快速走完的思考順序**,避免在慌亂中跳過關鍵的那一個問題。
...
- 新格式資料已寫入生產: 是
  ⟶ 部分匯率記錄已按新解析格式寫入(JSON 結構從 flat 改為 nested)
<!-- 為什麼這欄:「沒有 migration」不代表可以安全回滾。
     這次雖然沒有 ALTER TABLE,但資料格式已改變,舊程式碼讀到新格式的 JSON 會解析失敗。
     這是最容易被跳過、也最危險的一問。 -->
- 舊版程式碼讀得了已寫入的新格式: 否
  ⟶ 舊程式碼期待 flat JSON,新格式是 nested,讀取會丟 KeyError
- 有無外部副作用已觸發: 否(問題在讀路徑,交易未過帳)

[第三問:修正代價]
- 根因已確認: 是(舊格式匯率資料被新解析邏輯覆寫後,舊程式碼讀不了)
...
- **上一章**:[第 22 章｜藍綠與金絲雀部署](./ch-22-blue-green-canary.md) ⸺ 藍綠部署的 traffic 切換正是回滾能力的一種實現
- **下一章**:[第 24 章｜資料庫遷移與零停機變更](./ch-24-db-migration.md) ⸺ 資料相容性問題的完整解法
- **強連結**:[第 21 章｜Feature Flag 與漸進式發布](./ch-21-feature-flags.md) ⸺ Feature flag 是另一種「可快速關閉」的前向修復手段
- **強連結**:[第 26 章｜從告警到根因:生產環境除錯](../part-06-operations/ch-26-alert-to-rootcause.md) ⸺ 根因找到之後,才是 hotfix 的起點
- **強連結**:[第 28 章｜On-call 與事故處理](../part-06-operations/ch-28-on-call.md) ⸺ 回滾決策卡應放進 on-call runbook
