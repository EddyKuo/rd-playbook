...
> **前置閱讀**:[第 8 章｜控制複雜度](./ch-08-complexity.md)、[第 6 章｜可讀性:為下一個人而寫](./ch-06-readability.md)
> **下游章節**:[第 10 章｜程式碼層的技術債](./ch-10-tech-debt.md)、[第 12 章｜單元測試與 TDD 的落地](../part-03-testing/ch-12-unit-tdd.md)

## 9.1 共感現場:「我知道這裡很亂,但現在不是時候」
...
於是,新功能一個一個疊上去,函式從一百多行長到兩百行,再到現在的三百多行。每次加功能的人都說:「等這波忙完再來整理。」但那個「忙完之後」從來沒有到來。
...
## 9.3 一起做判斷:四個原則讓你知道什麼時候該動手、怎麼動手
...
### 9.3.1 「三次法則」:重複到第三次,就是重構的訊號

有個很好記的原則叫「三次法則」(Rule of Three),出自 Martin Fowler 的《Refactoring》。

第一次實作某個邏輯:直接寫,不需要考慮是否會被重用。
第二次遇到類似邏輯:覺得可以抽,但也可以先忍著。
第三次遇到類似邏輯:**停下來,抽它**。

為什麼是三次而不是兩次?因為「兩個地方有一樣的東西」可能是巧合;「三個地方有一樣的東西」幾乎一定是設計的問題。而且三次之後你也對這個邏輯夠熟了,知道它真正的邊界在哪裡,抽象的時候不容易抽錯。

在小葦的 MedChart 案例裡,三次法則的訊號其實很早就出現了。第四個藥品類別上線的時候,驗證邏輯的某個片段就已經在三個地方出現了:一次在 `_validate_antibiotic()`,一次在 `_validate_controlled()`,一次在剛加的新類別裡。但那時候沒有人意識到這是「第三次」——原因有兩層。一層是這些重複的片段散落在函式的不同角落、埋在不同藥品類別的區塊裡,要把它們聯想在一起、看出「這其實是同一段邏輯」,需要額外的心理負擔,而不是一眼就能看見。另一層是每次加新類別的時候,手上都有一個火燒眉毛的上線期限,沒有人有餘裕停下來問「這是不是第三次了」。於是到了第十七個類別的時候,重複的片段已經散落在整個函式的各個角落了。
...
這個「刻意改錯」的步驟叫做「變異測試」(Mutation Testing)的手動版本。你不需要工具,手動做就夠了。在 MedChart 的例子裡,你可以試著把 `validate_expiry()` 的 `>= today` 改成 `> today`,看看有多少個測試因為邊界條件改變而紅掉。如果只有 1 個或 0 個測試紅掉,代表現有測試對這個邊界的保護是空的——你以為自己有安全網,其實只是一張看起來像安全網的布。
...
在 MedChart 的例子裡,小葦發現的正是這個問題:現有的 8 個整合測試覆蓋了 happy path,但都沒有測試「空白藥品名稱」和「過期日期格式錯誤」這兩個邊界條件。她做了刻意改錯的驗證,把 `validate_expiry()` 的日期格式判斷故意改錯,只有 1 個測試變紅——這代表什麼?它代表這個判斷在大多數測試路徑裡根本沒有被跑到,其餘 7 個測試看到的都是別的邏輯分支,對這裡完全沒有保護作用。
...
這個是近年才出現的狀況。你把函式貼給 AI,說「幫我整理一下」,它給你一個看起來更乾淨的版本。你覺得邏輯差不多,就貼上去了。但「看起來一樣」和「行為完全相同」之間,可能有很細微的差距——比如邊界條件的處理、例外的拋出方式、null 值的行為、浮點數比較的細節——而這些差距在 happy path 的測試裡不會顯現。比如今天是 2026-07-03,原版的規則是 `expiry_date >= 2026-07-03`,允許今天的處方視為有效;AI 把它改成「藥品有效期晚於今天」,寫成 `expiry_date > 2026-07-03`,看起來邏輯相同,但今天的處方會被判定為過期。這一天的差異,在醫療系統裡可能意味著病人今天到藥局領不到本該有效的處方。
...
**絆倒處五是一個特殊情況:如果你不得不面對沒有測試的遺留程式碼,該怎麼辦?**

跟前面四個「不要這樣做」的提醒不一樣,這一個沒有簡單的迴避方式——它在現實的工作環境裡非常普遍,而且往往無法選擇不接手。你接手一個沒有測試的老模組,你知道它需要重構,但你也知道如果先補測試可能要花好幾天。這個兩難怎麼辦?
...
三個禮拜後,那個函式從 312 行拆成了三個各 35-50 行的子函式,加上一個 30 行的入口函式。測試從 8 個長到了 16 個,覆蓋了之前的空洞。
...
- **下一章**:[第 10 章｜程式碼層的技術債](./ch-10-tech-debt.md) ⸺ 重構是償還技術債的手段,下一章談怎麼辨識、量化與記錄它
- **前置閱讀**:[第 8 章｜控制複雜度](./ch-08-complexity.md) ⸺ 圈複雜度過高是觸發重構的最常見訊號之一
- **強連結**:[第 6 章｜可讀性:為下一個人而寫](./ch-06-readability.md) ⸺ 重構的成果很多時候就是可讀性的提升
- **強連結**:[第 12 章｜單元測試與 TDD 的落地](../part-03-testing/ch-12-unit-tdd.md) ⸺ 本章說安全網要在,第 12 章說怎麼鋪
- **強連結**:[第 38 章｜審查 AI 生成的程式碼](../part-08-ai-era/ch-38-reviewing-ai-code.md) ⸺ AI 重構的結果同樣需要測試守護
