...
> **前置閱讀**:[第 6 章｜命名、抽象與邊界](./ch-06-naming-abstraction.md)
> **下游章節**:[第 8 章｜重構的時機與安全網](./ch-08-refactoring.md)

## 7.1 共感現場:那個「改了三次還是壞掉」的判斷邏輯
...
## 7.2 真正的問題:複雜度不是「條件多」,而是「可能路徑爆炸」

我們把小雅那段程式碼慢慢拆開來看。

表面上,它的條件數量並不多:三種 role、三種 plan、幾個 feature 屬性。但如果你把所有可能的輸入組合數出來,光是 `admin` 這條路就已經有超過十種不同的執行路徑——而且有些路徑看起來幾乎一樣,實際上卻走到不同的結果。
...
你會發現這樣一來,主函式裡的每條路徑都很短——走到底只需要追蹤很少的前提。「走到這裡,我知道什麼是真的」這種感覺,就是提早 return 在幫你做的事。

### 7.3.2 把巢狀拆平:每一層只做一件事
...
### 7.3.3 用資料驅動代替條件樹:當「很多個 if 在做同一類判斷」時

有一種複雜度很隱蔽:不是一個大函式裡的深層巢狀,而是「十個 if-elif 在判斷同一種事」。這時候,可以考慮把判斷規則從程式碼裡抽出來,用資料來表達。
...
這樣一來,「新增一個方案」不需要在邏輯裡加一個 elif——只需要在 `PLAN_RULES` 裡多加一行。這是資料驅動設計帶來的可維護性收益。

### 7.3.4 馴服狀態爆炸:把「狀態」和「行為」分開想

小雅那段程式碼還有另一個問題:它把多個物件的狀態(user.role、tenant.plan、feature 的各種屬性)交叉組合在同一個函式裡做判斷,導致可能的狀態組合數成幾何級數成長。
...
下面幾個地方很多人都走過——不是因為不夠仔細,而是因為程式碼本身正在要求你的大腦同時記住太多事。每一位認真的工程師幾乎都在這裡跌過一跤,這是這種複雜度的副作用,不是個人的問題。認識它們,只是為了下次遇到的時候,能比以前快一點認出那個感覺。
...
### 7.5.1 範例:Nexthread 的 `can_access_feature` 重構前評估
