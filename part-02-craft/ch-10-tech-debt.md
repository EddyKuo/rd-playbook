...
> **前置閱讀**:[第 9 章｜重構的時機與安全網](./ch-09-refactoring.md)
> **下游章節**:[第 11 章｜可測試的程式碼設計](../part-03-testing/ch-11-testable-code.md)

## 10.1 共感現場:那個「等我有空再整理」的地方
...
### 10.3.2 記錄:技術債台帳(Tech Debt Ledger)
...
欄位設計之所以長這樣,是因為「記得住」和「說得清」是兩回事。比方說,同一筆債,用兩種寫法記下來,後續的命運完全不同:

- ❌ 模糊的記錄:「`OrderService` 有性能問題」
- ✓ 清晰的記錄:「`OrderService.processOrder()` 每月回應時間 +8ms,月均多花 12 小時開發成本」

第一種寫法,只有寫下它的工程師自己知道有問題——換了人接手,這句話就只是一個模糊的警告,沒有人敢動,也沒有人知道該不該動。第二種寫法,PM 一眼就能看出:這個月需要投入約 1.5 人天,才能止住這個地方的出血。同一件事,差別只在於有沒有把「位置」和「利息」寫成具體的數字——這正是台帳欄位設計的用意:逼自己在記錄的當下,就把模糊的感覺換成看得懂的語言。
...
BuyNow 的這個案例之所以同時看了三個維度,是因為它們各自推動了不同的決定:「開發速度利息」推動的是技術債 vs 其他功能需求的優先序討論——12 小時的月均利息,足以讓它排進 sprint;「可靠性利息」則觸發了「必須在下次大促前處理」的緊迫性,因為 48ms 的累積已經逼近棄單率上升的閾值;「安全性利息」在這個案例裡幸好不適用,但特別列出來是為了提醒:換一個情境,比如外部依賴帶著未修補的 CVE,安全性利息可能才是最高的那一個,要優先看。

也就是說,BuyNow 需要三個維度並看,是因為它的債同時牽涉到開發效率和線上可靠性;換成別的團隊、別的債,情況可能簡單得多。你的團隊不需要每次都三個維度全算——選一個最能推動行動、你說得出具體數字的維度,就足以把討論從「感覺」拉進「數字」。
...
不管是哪一種類型,原則都一樣:不需要每種利息都算,找到那個「一算出來,大家就會點頭」的數字,就夠了。

### 10.3.4 決策框架:何時還、何時暫緩
...
上面的表格,是快速判斷用的「直觀表」——手上有一筆債,對照兩個維度,馬上能落在哪個象限。但如果要更細緻地跟蹤一筆債從「被發現」到「被還清」的完整過程——尤其是暫緩之後的定期重評估——就需要一個更完整的流程圖,貼在團隊的 Wiki 或 Jira 上,讓每個人都能照著走。它把順序講得更清楚:先辨識類型、記錄台帳,再判斷「觸發代價」的高低,最後才對照「還債成本」做出決策,這和表格的兩個維度是同一件事,只是拆成了先後兩步:
...
這個流程有一個容易被跳過的節點:**「暫緩」不是終點**——圖裡把它接回「觸發代價」的判斷,表示暫緩的技術債需要定期重評估,因為觸發代價會隨著時間和業務壓力升高。一筆今天觸發代價低的債,六個月後可能已經升到不得不還的位置了。
...
## 10.5 帶得走的工具 ⸺ 一頁式「Tech Debt Ledger 卡」
...
### 10.5.1 範例:BuyNow 的 `processOrder()` 技術債卡
...
- **上一章**:[第 9 章｜重構的時機與安全網](./ch-09-refactoring.md) ⸺ 還債的執行手法,重構是最主要的工具
- **下一章**:[第 11 章｜可測試的程式碼設計](../part-03-testing/ch-11-testable-code.md) ⸺ 可測試性是預防位移債的設計決策
- **強連結**:[第 1 章｜為什麼工程實作需要決策框架](../part-01-foundations/ch-01-why-engineering-decisions.md) ⸺ 技術債的核心也是「判斷」問題:欠了多少、利息多高、何時還
- **強連結**:[第 17 章｜Code Review:看什麼、怎麼給回饋](../part-04-collaboration/ch-17-code-review.md) ⸺ Code Review 是及早發現新增技術債的關口
