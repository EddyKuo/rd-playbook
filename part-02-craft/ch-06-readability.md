...
> **下游章節**:[第 7 章｜命名、抽象與邊界](./ch-07-naming-abstraction.md)——可讀性的下一層:命名決策的細節

## 6.1 共感現場:那段「昨天還看得懂」的程式碼
...
這就帶出了第 6 章真正想問的問題:可讀性到底是什麼,又該怎麼讓它撐得過時間?

## 6.2 真正的問題:可讀性不是文風,是下一個人的時間成本

我們把這件事慢慢拆開來看。

很多工程師對「可讀性」的直覺感受是:「要寫得清楚一點」「要加一些好的命名」——這個感受對,但還沒有看到問題的根。真正的問題是:**可讀性是一個時間跨度的問題,不只是當下的一個文風問題。**

順著這個道理,隱性脈絡會隨著時間和人員變化而逐漸消失。讓我們看看在不同時間點上,讀者面對的是什麼——我們可以把程式碼的「讀者」分成三個時間點來看:
...
也就是說,「能讀懂」其實是一個連續的衰減——每過一段時間、每換一個人,讀者腦袋裡可用的「隱性脈絡」就少一層,文字本身就得多承擔一點責任。也就是說,隱性脈絡會逐漸消失,程式碼本身就必須承擔更多說明責任。這正是第 1 章「交付前第④問」所守護的——當我們問「三個月後的人讀得懂嗎?」,不是在要求每個人都把程式碼寫成教科書,而是在問:當那份隱性脈絡消失之後,這段程式碼還能讓人看出「它在做什麼、為什麼這樣做」嗎?

那麼問題來了——程式碼靠什麼來補償隱性脈絡的消失呢?很多工程師的直覺反應是「加更多注釋」,但其實答案不是「多寫一點注釋」那麼簡單。可讀性其實是三件事的組合:好的命名讓人看出「這個東西是什麼」,好的結構讓人看出「這段邏輯在做什麼」,必要的注釋讓人看出「為什麼要這樣做」。這三件事缺一不可,但它們有不同的優先順序,也有不同的使用時機。

## 6.3 一起做判斷:命名、結構、注釋各司其職
...
### 6.3.4 注釋:只說「為什麼」,不說「什麼」

註釋其實很容易被用錯地方。我發現很多工程師在寫註釋的時候,往往卡在這裡——所以我想先講最常見的兩種誤用。注釋有兩種,效果完全不同:

**解釋「什麼」的注釋**(往往是多餘的):

為什麼多餘?因為這種註釋要解決的問題,其實應該由命名和結構來解決。如果命名夠好,它就多餘;如果命名不好,它也救不了根本問題。來看一個例子:
```python
# 計算加權分數
x = p['b'] * 0.4 + p['c'] * 0.3 + t * 0.2 + v * 0.1
```
這行注釋說的正是 `x = p['b'] * 0.4 + ...` 這行程式碼本身已經在做的事——它沒有補上任何程式碼讀不出來的東西。
...
這是在函式體裡很常見的一種:為了「少一個中間變數」,把三件事串成一行。特別是對剛學過函式式編程(functional programming)的工程師,容易把多個操作串成一行以顯得簡潔:
...
- **下一章**:[第 7 章｜命名、抽象與邊界](./ch-07-naming-abstraction.md) ⸺ 命名決策的細節展開
- **強連結**:[第 1 章｜為什麼工程實作需要決策框架](../part-01-foundations/ch-01-why-engineering-decisions.md) ⸺ 本章展開的是第④問「別人讀得懂嗎」
- **強連結**:[第 9 章｜重構的時機與安全網](./ch-09-refactoring.md) ⸺ 可讀性改善與重構的關係
- **強連結**:[第 17 章｜Code Review:看什麼、怎麼給回饋](../part-04-collaboration/ch-17-code-review.md) ⸺ 可讀性是 Code Review 的核心審查面向
- **強連結**:[第 38 章｜審查 AI 生成的程式碼](../part-08-ai-era/ch-38-reviewing-ai-code.md) ⸺ AI 生成的程式碼往往通過語法卻在可讀性上打折
