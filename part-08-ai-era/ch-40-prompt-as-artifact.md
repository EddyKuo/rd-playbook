...
## 40.2 真正的問題:prompt 是程式碼,context 是狀態
...
## 40.3 一起做判斷:把 prompt 當成可維護的工程產物

好,我們確認了 prompt 需要被當成工程產物對待。那實際上該怎麼做?

一個好用的角度是——你可以把 prompt 想成一種特殊的「配置文件(configuration)」,它有自己的結構、有輸入變數、有預期的輸出格式,需要被測試、被版控、被文件化。

### 40.3.1 Prompt 的工程生命週期
...
對於高優先度的 case,用第二個 LLM 呼叫當「裁判」:「這個輸出有沒有回答 X?有沒有包含 Y?」這層測試要寫清楚評分標準,也要接受一定的模糊性——你在測試的是語意品質,不是字元比對。
...
### 40.3.4 Context 作為資產:建立「知識庫」而非「每次重新貼」
...
### 40.3.5 Prompt 版控在 CI/CD 流水線中的實踐
...
### 40.5.1 範例:CloudPost 的 PR 摘要 prompt
...
purpose: |
  ...
  purpose 欄位說清楚「在哪裡被呼叫」,讓維護者知道修改這支 prompt
  會影響哪個自動化流程,而非單純的手動輔助工具。
```
...
user_template 裡明確要求「無明顯風險請明確寫出」,是因為早期版本(v1.0)讓模型直接省略風險欄位,導致 code reviewer 誤以為「空的就是沒問題」。明確要求模型在沒有風險時也要表態,消除了這個誤讀。
