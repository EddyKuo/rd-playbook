...
# 第 40 章｜Agentic 開發:讓 Agent 跑工程任務
## ⸺ 當程式碼開始自己跑起來,你的護欄在哪裡?

> **前置閱讀**:[第 38 章｜審查 AI 生成的程式碼](./ch-38-reviewing-ai-code.md)　[第 39 章｜為 AI 生成碼補測試與防護](./ch-39-testing-ai-code.md)
> **下游章節**:[第 41 章｜Prompt 與 context 作為工程產物](./ch-41-prompt-as-artifact.md)　[第 42 章｜AI 時代的工程師心智與責任界線](./ch-42-engineer-mindset.md)

---

## 40.1 共感現場:那個凌晨把 PR 開好的 Agent
...
當 agent 能自主執行多個步驟、每一步都有真實副作用時,你就無法再在每一個決策點上確認它的判斷——它讀檔、下決定、改程式碼、跑測試、再決定下一步,整條鏈子跑完你才看得到結果。這樣的「自主執行鏈」帶來一個你在傳統輔助編碼裡不會遇到的問題:**你不在每一個決策點上。**
...
正因為這樣,護欄的核心作用就是把「每一步都在賭」變成「每一步都能確認」——讓它做壞了能退(降低單次風險)、做到邊界會停(防止超出預期)、做完了你能看清楚發生了什麼(事後可追溯)。也就是說,agentic 開發的核心不是「讓 agent 更聰明」,而是:**在 agent 能力既定的前提下,你怎麼設計這樣的護欄**。

這就是本章要和你一起想清楚的事。

---

## 40.3 一起做判斷:三層護欄,讓 Agent 工作得有邊界
...
把邊界和可逆性定清楚之後,接下來的問題是:agent 執行的過程中,每一步應該怎麼被約束和觀察?這就進入了執行層的三個護欄。

### 40.3.2 執行層:工具限縮、人類在環、執行日誌
...
有了工具限縮之後,還需要在執行的每一步加上人的確認點,確保 agent 不會在你沒注意到的地方做出錯誤決策。
...
**執行日誌(Execution Log)**:agent 做了哪些步驟、讀了哪些檔案、執行了哪些指令,都應該被記錄下來並且你看得到。這不是為了事後審計,而是為了**當事情出了問題,你能快速定位是哪一步開始走偏的**。沒有執行日誌的 agent,就像一個半夜幫你工作、但你完全不知道他做了什麼的實習生——結果對了還好,結果錯了,調查和追查問題的根源會變得極其困難。

### 40.3.3 交付層:可觀察的 diff、驗收標準、回滾路徑
...
更微妙的是:agent 自己也會補測試。因為 agent 只能從程式碼和測試本身學習,對系統裡那些「沒寫在文件、但客戶已在依賴」的暗契約沒有理解,所以它補的測試往往是覆蓋它改動的部分,而不是覆蓋「改動可能波及到的部分」。這兩件事差很遠。前者讓你的 CI 更綠,但未必讓你的系統更安全。
...
### 40.5.1 範例:Loomify 的 rate limit 改版任務,如果當初有這張卡
...
- **前一章**:[第 39 章｜為 AI 生成碼補測試與防護](./ch-39-testing-ai-code.md) ⸺ 測試是 agentic 任務驗收標準的基礎
- **下一章**:[第 41 章｜Prompt 與 context 作為工程產物](./ch-41-prompt-as-artifact.md) ⸺ 任務設計卡的 prompt 本身也值得版控
- **強連結**:[第 38 章｜審查 AI 生成的程式碼](./ch-38-reviewing-ai-code.md) ⸺ 交付層的 diff review 技術
- **強連結**:[第 42 章｜AI 時代的工程師心智與責任界線](./ch-42-engineer-mindset.md) ⸺ 本章護欄背後的責任觀
- **強連結**:[第 13 章｜契約測試與整合測試](../part-03-testing/ch-13-contract-integration.md) ⸺ 外部消費者契約測試的實作技術
- **強連結**:[第 24 章｜回滾與前向修復決策](../part-05-delivery/ch-24-rollback.md) ⸺ 可逆性評估的下游操作
