...
# 第 39 章｜為 AI 生成碼補測試與防護
## ⸺ 當程式碼快到讓人來不及懷疑,測試就是你和生產環境之間的最後一道牆

> **前置閱讀**:[第 38 章｜審查 AI 生成的程式碼](./ch-38-reviewing-ai-code.md) ⸺ 先知道看什麼,再知道怎麼守
> **下游章節**:[第 40 章｜Agentic 開發:讓 agent 跑工程任務](./ch-40-agentic-dev.md) ⸺ agent 大量產出之後,測試策略就更重要了

---

## 39.1 共感現場:「它生出來的,應該不會有問題吧?」
...
正因為這個細節——費率表配置是否完整——只存活在 Paytide 的組織記憶和系統歷史裡,而不在 prompt 的文字裡,AI 根本沒有管道看到它。這帶出了這章最核心的問題:**當 AI 幫你快速填滿了「正常路徑」,誰來守住那些它不知道自己遺漏的邊界?**

答案是你。但這不是一件靠眼力就能做到的事——它需要一套刻意設計的補測框架,幫你把那些 AI 看不到的地方,一一變成明確的測試斷言。接下來我們一步一步來看。

---

## 39.2 真正的問題:AI 生的程式碼有一種特別的「似是而非」
...
人手寫到一個困難的地方會停、會皺眉,那個遲疑是你和問題之間的接觸點。你停頓的地方,通常正好是問題的所在。AI 沒有遲疑——它一路順滑地把你帶過了問題點,讓你的眼睛滑過去了。

小薇看 AI 生成的費率計算程式碼,眼睛沿著 `fetch_fee_rate()` → `rate * amount` → `total + fixed_fee` 一路流暢地走下去,沒有任何東西讓她停頓。「費率如果是 `None` 的時候呢?」這個問題,從來沒有成為一個讓她停下來的問題,因為眼睛從來沒有停在那個地方。

AI 的程式碼提供了流暢性,同時也消除了本來應該讓你停下來問邊界問題的那個時刻。結果是——你眼睛沒停,問題也就沒被問出來。

### AI 讓「測試動機」變弱了

還有另一個微妙的心理效應。人手寫完一段程式碼之後,通常會覺得「我寫的,不一定可靠,要測一下」。這種不確定感其實是好的——它是驅動你去補測試的動力。

AI 生成的程式碼看起來那麼乾淨、那麼結構化,讀起來像是某個資深工程師寫的,你很容易默默覺得「這個應該沒問題吧」。這不是迷信,而是一種認知捷徑——在你的經驗裡,漂亮結構的程式碼通常是資深工程師精心打磨過的,所以你會預設這樣的程式碼已經把邊界情況考慮周全了。看起來像是好程式碼的東西,通常就是好程式碼。
...
這是一個容易被忽略的現象:兩段 AI 生成的程式碼相互驗證了彼此的實作,測試因此通過了——但通過的是 AI 對規格的詮釋版本,不是你實際需要的業務行為。
...
- **上一章**:[第 38 章｜審查 AI 生成的程式碼](./ch-38-reviewing-ai-code.md) ⸺ 審查與補測是同一件事的兩面:審查幫你找到哪裡需要測,測試讓審查的結論落地
- **下一章**:[第 40 章｜Agentic 開發:讓 agent 跑工程任務](./ch-40-agentic-dev.md) ⸺ agent 會大量產出程式碼,這章的補測策略就是你的防護網
- **強連結**:[第 11 章｜可測試的程式碼設計](../part-03-testing/ch-11-testable-code.md) ⸺ 補測的前提是程式碼可測;AI 生成碼不一定天生可測
- **強連結**:[第 13 章｜契約測試與整合測試](../part-03-testing/ch-13-contract-integration.md) ⸺ 整合邊界的測試替身策略在這裡有完整討論
- **強連結**:[第 14 章｜測試替身的取捨](../part-03-testing/ch-14-test-doubles.md) ⸺ fake / stub / spy 的選擇直接影響補測的成本與信心
