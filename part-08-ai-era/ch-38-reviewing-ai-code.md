...
> **前置閱讀**:[第 37 章｜AI 輔助編碼的工作流重塑](./ch-37-ai-assisted-coding.md) ⸺ 了解 AI 如何融入開發流程
> **下游章節**:[第 39 章｜為 AI 生成碼補測試與防護](./ch-39-testing-ai-code.md) ⸺ 審查之後的防護網

## 38.1 共感現場:「它測試都有過,應該沒問題吧」
...
除了規模邊界和領域特殊性,AI 還有第三個結構性的盲區,這一次它不是不知道你的環境,而是不知道你沒有明說的「應該做但沒被要求」的事。
...
也就是說,AI 其實相當忠實於你提供的描述。正因為這份忠實,AI 只會實現你明確說出來的部分,沒有明確說出的就完全不會做——問題是,那份描述幾乎從來不完整。不完整不是你的錯,而是因為很多事情我們以為是理所當然、根本想不到要說出來。
...
- 一年後估算:大商戶的待結算筆數可能長到 2 萬筆。這時候,`user_id` 上如果沒有對應的索引,資料庫就必須全表掃描——2 萬筆的全表掃描,加上月結日 2,000 個並發查詢,足以讓資料庫的查詢佇列開始塞車。
...
第一個版本看起來「很有彈性」,但三個月後的工程師要讀懂它,需要先找到 `StatusRenderer` interface、找到 `DefaultStatusRenderer` 結構、找到 `NewStatusRenderer` factory——才知道它只是在做一個 switch。而第二個版本一眼就看完了。

這裡的判斷方式不是「interface 好不好」——interface 本身沒有錯,問題在於它現在有沒有存在的理由。有一個很實際的問法:「在你的現有系統裡,有沒有其他地方也需要用不同方式渲染訂單狀態,比如同時要輸出純文字、HTML、還有通知訊息三種格式?」如果答案是有,`StatusRenderer` 這個抽象就是值得留下的彈性;如果整個系統目前只有這一處會呼叫它,唯一的實作也只有 `DefaultStatusRenderer`,那麼這層抽象就只是在替一個未來可能永遠不會發生的需求,提前付出理解成本。如果去掉它,功能還能正確運作,而且更好讀,那就去掉。

> **修正方向**:問自己「這個抽象現在是必要的,還是它只是 AI 的習慣動作?」具體的檢驗方法是回頭看你的系統:有沒有其他地方已經在用同樣的抽象模式?如果沒有,它很可能只是 AI 的習慣,而不是你系統真正的需求。簡單是一種主動選擇,不是妥協。抽象是有代價的——每一層抽象都需要日後的工程師去理解、追蹤、維護。只在有具體理由時加它。
...
- **前一章**:[第 37 章｜AI 輔助編碼的工作流重塑](./ch-37-ai-assisted-coding.md) ⸺ 了解 AI 如何融入開發流程,是本章審查的前提
- **下一章**:[第 39 章｜為 AI 生成碼補測試與防護](./ch-39-testing-ai-code.md) ⸺ 審查通過只是第一步,補測試才讓它真正安全
- **強連結**:[第 1 章｜為什麼工程實作需要決策框架](../part-01-foundations/ch-01-why-engineering-decisions.md) ⸺ 交付前四問是本章六個維度的基礎
- **強連結**:[第 17 章｜Code Review:看什麼、怎麼給回饋](../part-04-collaboration/ch-17-code-review.md) ⸺ 本章的六維框架是 Code Review 在 AI 場景下的延伸
- **強連結**:[第 46 章｜判斷力的養成:當階梯被 AI 抽掉](../part-09-synthesis/ch-46-cultivating-judgment.md) ⸺ 本章談「如何審查」,第 46 章談「如何養成審查的能力」
