...
> **前置閱讀**:[第 32 章｜瓶頸定位(CPU/IO/網路/鎖)](./ch-32-bottleneck.md)、[第 31 章｜效能量測先於優化](./ch-31-measure-first.md)
> **下游章節**:[第 34 章｜並發、非同步與背壓](./ch-34-concurrency.md)、[第 35 章｜容量規劃與壓力測試](./ch-35-capacity.md)

## 33.1 共感現場:「加了快取,但系統還是慢」
...
### 33.3.1 快取的層次圖:先選放哪,再選怎麼快

一個請求從瀏覽器到資料庫,會經過以下幾個位置。每個位置都可以放快取,但適合擋的東西不一樣。

在選擇層次之前,有一個思維轉換很有幫助:把快取不想成「加速工具」,而是「需求攔截器」。每往更前面的層次移動一步,能攔截的需求量就成倍增加——CDN 可以攔截全球分散的讀取需求,API Gateway 可以攔截熱門公開查詢,應用層 Redis 可以攔截 DB 讀取壓力。問「這份資料應該在哪一層被攔截?」比問「我應該用什麼快取工具?」更能讓你找到正確的起點。
...
### 33.3.2 失效策略的選擇:TTL、主動清除、還是事件驅動
...
適合:可以接受短暫過期的唯讀或低寫入資料——例如商品描述、促銷條文、推薦清單。
...
```mermaid
flowchart TD
  A["這份資料需要快取嗎?<br/>(先量測,第31章)"] -->|讀取瓶頸確認| B{"資料的一致性<br/>要求?"}
...
看到「不適合快取」這個節點,你可能會覺得有點奇怪——走到這裡是不是代表我們什麼都做不了?其實不是這樣的。「不快取」本身就是一個完整的策略選擇,而不是設計的缺失。當一份資料的一致性要求很高(例如庫存數量在搶購時刻不能有任何誤差),讓每次讀取都直接查資料庫,是對這個欄位最誠實的處理方式——它確保讀者拿到的永遠是最新狀態,代價是多一次 DB 查詢。這個代價通常比因為過期快取導致超賣要小得多。你可以針對「高一致性要求的欄位」考慮 read-through 模式(讀取時同步刷新快取)、或者接受它不快取,同時把精力放在優化這個欄位的資料庫查詢效率上。「不快取某個欄位」和「不懂快取」是兩件完全不同的事。

### 33.3.4 三大快取危機的防護:雪崩、穿透、擊穿

快取一旦被大量請求依賴,就出現了三種常見的危機。它們名字相近,但成因和解法不同,值得仔細分清楚。
...
被過期資料害到投訴之後,有人的第一個反應是「那我把 TTL 改成 30 秒」或「那乾脆不快取這個欄位」。但這兩個做法都沒有解決根本——30 秒 TTL 只是把過期資料的窗口縮小了,沒有讓它消失。
...
- **上一章**:[第 32 章｜瓶頸定位(CPU/IO/網路/鎖)](./ch-32-bottleneck.md) ⸺ 確認瓶頸在資料庫讀取後,再考慮快取
- **下一章**:[第 34 章｜並發、非同步與背壓](./ch-34-concurrency.md) ⸺ 快取解決了讀取瓶頸,並發設計處理的是同時大量寫入與請求的問題
- **強連結**:[第 31 章｜效能量測先於優化](./ch-31-measure-first.md) ⸺ 量測是決定「哪裡需要快取」的前提
- **強連結**:[第 35 章｜容量規劃與壓力測試](./ch-35-capacity.md) ⸺ 快取 miss 時 DB 能承受的 QPS,需要壓測才知道
