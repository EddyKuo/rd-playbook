...
這和亂槍打鳥的差別在於:**假設是有因果依據的**。「CPU 高 → 可能是序列化成本」「I/O wait 高 → 可能是全表掃描」,這些假設都來自對系統因果關係的理解。有了假設,你才知道要去哪裡找證據;沒有假設,所有工具的輸出都只是一堆數字。

「假設 → 驗證 → 修正」這個循環有一個關鍵細節:驗證的方式要「最小成本」。什麼叫最小成本?就是「能用一個指令回答的問題,不要用部署來回答」。比如懷疑是慢查詢,先看 slow query log 裡最近十分鐘的記錄;懷疑是連線池不夠,先看 `SHOW STATUS LIKE 'Threads_connected'`;懷疑是外部 API 慢,先在 Trace 裡找那幾個 span 的 p99。每一步都只比上一步多開一扇門,而不是一次把所有門都打開。
...
理解了定位的方法之後,你可能會想:「這樣應該就不會走彎路了。」其實不然——就算有了框架,幾個地方還是特別容易讓人打滑。下面這幾個地方,幾乎每個工程師在學效能定位的路上都會踩到一遍,提前認識它們,下次遇到就會少繞一圈。
...
開發環境和生產環境的差異很大:資料量不同、連線數不同、硬體規格不同、網路拓撲不同。很多在本機跑很快的東西,到了生產環境才暴露問題——就像[第 1 章｜為什麼工程實作需要決策框架](../part-01-foundations/ch-01-why-engineering-decisions.md)裡的小俊一樣。
...
讓我們回到小薇和 CartFlow 的故事。如果促銷後那天,她坐下來先填這張工作表,整個定位的過程大概會是這樣:
