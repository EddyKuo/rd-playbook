...
> **前置閱讀**:[第 35 章｜並發、非同步與背壓](./ch-35-concurrency.md)
> **下游章節**:[第 37 章｜AI 輔助編碼的工作流重塑](../part-08-ai-era/ch-37-ai-assisted-coding.md)

---

## 36.1 共感現場:大促前三天的那場會議
...
## 36.2 真正的問題:容量是一條線,流量是一個分佈

我們把這件事慢慢拆開。具體來說,那場會議的根本問題在於:你的系統提供的是一條固定的「容量上限線」,但實際的流量卻是一個分佈——而這個分佈的形狀會改變。日常是平坦的,大促時卻變成尖峰。要真正回答「能不能撐住」,你需要同時理解這三件事。
...
這些公式算出來的數字,不是精確預測,而是「你的危險邊界大概在哪」的粗估。也就是說,這四個數字當中,哪個最小,就代表哪個資源會最先達到飽和。因為整條鏈路的實際容量,永遠是由最短的那塊木板決定——不管其他環節有多強,超過這個最小值的時刻,系統就開始觸發瓶頸。所以壓測要「優先」從它開始施壓:把每個瓶頸的理論上限算出來,取最小的那個,那就是你整條鏈路最薄弱的一環。

### 36.3.3 第三步:設計有意義的壓力測試

壓力測試最常見的誤解是「把流量開到最大,看什麼時候掛」。這樣的測試能找到崩潰點,但無法告訴你在崩潰之前,系統的降級行為是什麼,也無法幫你做容量決策。

一個完整的壓測計畫,通常包含五種測試類型:
...
壓測工具方面,常見的選擇是 k6(Grafana 出品,腳本用 JavaScript 寫)或 Apache JMeter(老牌,配置靈活)。壓測工具有很多種,但關鍵不是選哪一個工具,而是能不能把你的「負載模型」忠實地翻譯成「壓測流量形狀」。舉個具體例子,拿前面那份「日常 200、大促啟動瞬間 2,400(12x)」的負載模型來說,用 k6 把它翻譯成尖峰測試的輪廓,大概長這樣:
...
## 36.5 帶得走的工具 ⸺ 一頁式「容量規劃與壓測計畫」

下面是一份空白的容量規劃與壓測計畫模板。它的目的不是讓你填一份文件交差,而是讓你在壓測之前,把那幾個「不問會吃虧」的問題先想清楚。欄位設計成可以貼進 Confluence、Notion 或 PR 描述裡的形式。下一節的填寫範例裡會包含開發者註解(用 `<!-- -->` 標注,說明為什麼要有這一欄),如果你要把它複製到實際專案裡,記得先把這些註解刪掉,保留乾淨的欄位就好。
...
- **上一章**:[第 35 章｜並發、非同步與背壓](./ch-35-concurrency.md) ⸺ 背壓機制是容量超出時的緩衝手段,兩章互為表裡
- **下一章**:[第 37 章｜AI 輔助編碼的工作流重塑](../part-08-ai-era/ch-37-ai-assisted-coding.md) ⸺ 進入 AI 時代研發的核心章群
- **強連結**:[第 32 章｜效能量測先於優化](./ch-32-measure-first.md) ⸺ 本章的壓測數據來源,需先讀懂量測
- **強連結**:[第 33 章｜瓶頸定位](./ch-33-bottleneck.md) ⸺ 瓶頸外推的詳細方法論
- **強連結**:[第 30 章｜SLO/錯誤預算的實作面](../part-06-operations/ch-30-slo.md) ⸺ 容量規劃的 SLO 門檻從這裡來
