...
不同類型的程式碼,入口藏的地方不同。下面這張表可以幫你在一兩分鐘內定位到它:
...
小雯畫出這張圖的時候,有一件事立刻跳出來了:`applyDiscount` 出現了兩次。一次在 controller 層,一次在 `saveSettlement` 的內部。也就是說,折扣金額被計算了兩遍——但兩次計算的時候,`order` 物件的狀態不一定一樣,因為中間的 `deductInventory` 改動了 `order.items` 的庫存欄位,而 `applyDiscount` 裡面有一段條件式依賴庫存數量來決定折扣金額。兩次計算、不同的中間狀態、不一樣的結果——bug 就藏在這裡。
...
小雯那個下午就是這樣過的:她從 `applyDiscount` 開始讀,進去之後看到一個 `calculatePromotionRule`,覺得有點複雜,跳進去看了看,裡面又呼叫了 `getActivePromotions` 和 `filterByUserSegment`……讀了快一個小時,她已經完全忘記她最初是來追折扣算錯的 bug 的。
