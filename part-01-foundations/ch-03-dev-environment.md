...
這個問題聽起來像「細節」,其實影響的是工程師一整天的工作節奏。認知心理學研究指出,人專注在一件事上的「心流狀態」很容易被打斷——而等待就是最常見的打斷來源。如果改一行邏輯要等三十秒才能看到結果,大腦就會自動切去查信件或刷社群媒體;等回來時,原本的思路已經斷了。讓回饋迴圈快,是為了保護你自己的專注力。
...
如果跑一次單元測試要四十秒,你不會每次改完都跑。如果是四秒,你會。`jest --watchAll` 或 Python 的 `pytest-watch`(開源)套件可以讓測試在每次存檔後自動執行,把測試從「想起來才跑」變成「寫完就自動跑」。
...
這是 Layer 1 最常見的退化路徑。一開始只有一個 DB,後來加了 Redis、Kafka、Elasticsearch、Jaeger,啟動一次要五分鐘,大家就開始跳過它直接連 dev server。環境設定有了,但沒人用,問題等於沒解。

特別值得注意的是這個退化過程非常隱性:沒有人決定「我們不用 compose 了」,只是有一天大家發現啟動很慢,然後悄悄地開始繞過去。等到真的有人注意到環境設定和現實已經分歧,代價往往比當初維護成本高出許多。
...
`.env` 裡有 secret,當然不能 commit。但「不能 commit secret」不等於「不能 commit 環境變數的鍵名」。一個 `.env.example`(只列出鍵名,值留空或填假值)commit 進 git,就能解決「不知道需要哪些變數」的問題,同時不洩漏實際的 secret。
...
更微妙的問題是:watch mode 開著之後,有時候會讓人產生「環境一直是最新的」的錯覺——但 watch mode 只能重啟服務,它不能保證你的 DB schema 是最新的、你的測試 fixtures 是對的、你的 stub 資料和最新的 API contract 是一致的。
...
- **強連結**:[第 14 章｜測試資料與測試環境](../part-03-testing/ch-14-test-data-and-env.md) ⸺ 本章談本機環境;第 14 章談測試環境的隔離與資料策略
- **強連結**:[第 15 章｜與 CI 整合的測試流水線](../part-03-testing/ch-15-ci-test-pipeline.md) ⸺ 本機的 pre-commit hook 和 CI 流水線是同一條防線的兩端
- **強連結**:[第 20 章｜CI/CD 流水線設計](../part-05-delivery/ch-20-cicd-pipeline.md) ⸺ 本機環境盡量對齊 CI 環境,才能讓「本機能跑」等於「CI 能跑」
