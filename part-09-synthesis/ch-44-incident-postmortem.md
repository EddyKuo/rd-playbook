...
> **前置閱讀**:[第 27 章｜從告警到根因:生產環境除錯](../part-06-operations/ch-27-alert-to-rootcause.md)、[第 29 章｜On-call 與事故處理(工程師視角)](../part-06-operations/ch-29-on-call.md)
> **後續章節**:[第 45 章｜接手 legacy 系統的 90 天計畫](./ch-45-legacy-90-days.md)、[第 46 章｜判斷力的養成](./ch-46-cultivating-judgment.md)

## 44.1 共感現場:那份你不確定該不該寫的復盤報告
...
知道事故發生了什麼之後,現在讓我們看看,如果是你在帶這次復盤,該從何處開始。
...
這份時間線透露了三件原本不明顯的事:
1. Priya 推送後的**三分鐘內**才開始出現告警——理想狀況下,系統應該在 DDL 操作剛開始(14:32)就發出警告,但卻延遲到 14:35 才觸發。這表明監控對「長時間進行中的 DDL 操作」沒有設置初期告警,只有在影響已經造成之後,才從結果面(支付失敗率 > 5%)察覺問題,而不是從操作本身察覺。
...
在這個例子中,上面的 Mermaid 甘特圖礙於篇幅,只能呈現時間軸和動作本身,沒辦法把「當時看到的資訊」也畫進去,所以才另外用文字列表補充。但在你實際撰寫復盤報告的時候,不需要受限於圖表的篇幅——應該像 §44.5.1 的範例那樣,在時間線的每一行後面用「⤷ 當時的資訊是……」逐一標註,讓讀者不用另外對照文字說明,就能看懂每個決定背後的判斷依據。

### 44.3.3 第二步:五問根因(5 Whys)
...
「因為工程師疏失」是五問根因的死胡同,因為「疏失」是對一個人個人特質的描述,沒辦法轉化成系統性的改善行動——你沒辦法系統性地「讓人不疏失」,但你**可以**系統性地「改變讓人容易疏失的環境」。所以每次根因指向「人」的時候,再多問一步:「什麼環境讓這個人容易做出這個決定?」
...
「改善監控」「加強文件」是最常見的偽行動項——它聽起來正確,但一個行動項必須同時明確(1)誰負責、(2)什麼時候完成、(3)完成的樣子是什麼,三者缺一,執行力就會落空。少了負責人,沒人覺得這是自己的事;少了期限,永遠可以往後排;少了完成標準,就算有人動手,也不知道做到什麼程度才算做完。復盤會結束之後,這些項目就默默消失了。
...
為什麼是這幾個欄位?因為一份好的復盤報告要回答三個讀者的問題:管理層想看「**範圍與代價**」(摘要);當事工程師與 on-call 想看「**事情是怎麼發展的**」(時間線);未來的工程師想看「**怎麼讓這件事不再發生**」(行動項 + 我們學到了什麼)。少掉摘要,管理層無法快速評估商業影響和決策優先級;少掉時間線,on-call 工程師無法快速理解事故的進展和當時的資訊限制;少掉行動項和「我們學到了什麼」,未來接手的工程師就得在下一次事故中重新踩一次坑。

### 44.5.1 範例:Clearflow 跨境支付阻斷事故(CASE-FIN-043)

我們回到 Clearflow 那次事故。如果當天復盤報告是這樣寫的,三個月後接手 on-call 的新同事就不需要在事故中重新學到「`CREATE INDEX CONCURRENTLY` 的存在」這件事了。

要提醒一件事:「我們學到了什麼」這一段的技術深度,應該根據報告的讀者範圍來調整。下面這個範例是寫給接手 on-call 或資料庫相關的工程師看的,所以直接點名 `pg_class` 這樣的實現細節;如果你的報告讀者更廣泛(例如同時涵蓋應用層、架構、產品),就應該把技術名詞往上收一層,改成類似「PostgreSQL 的 DDL 操作在大資料量表上會鎖住寫入,任何生產環境有超大表(1,000 萬筆以上記錄)的系統,DDL 操作都應該在變更評審階段特別檢查,並預留可能長時間停機的風險預案」這樣的寫法——保留因果,但不強求每個讀者都懂資料庫內部機制。
...
- **前章**:[第 43 章｜從設計到上線:一個完整功能的實作全紀錄](./ch-43-feature-end-to-end.md) ⸺ 功能走完全流程之後,下一步是從出錯的事故裡學習
- **下一章**:[第 45 章｜接手 legacy 系統的 90 天計畫](./ch-45-legacy-90-days.md) ⸺ 接手系統時,過去的事故報告是最重要的第一手資料
- **強連結**:[第 27 章｜從告警到根因:生產環境除錯](../part-06-operations/ch-27-alert-to-rootcause.md) ⸺ 除錯方法論是時間線重建的前置技能
- **強連結**:[第 29 章｜On-call 與事故處理(工程師視角)](../part-06-operations/ch-29-on-call.md) ⸺ 事故處理的現場操作,與復盤形成完整閉環
- **強連結**:[第 46 章｜判斷力的養成](./ch-46-cultivating-judgment.md) ⸺ 復盤是刻意養成判斷力最有效的機制之一
