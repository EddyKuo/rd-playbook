...
在這個例子中,上面的 Mermaid 甘特圖礙於篇幅,只能呈現時間軸和動作本身,沒辦法把「當時看到的資訊」也畫進去,所以才另外用文字列表補充。但在你實際撰寫復盤報告的時候,不需要受限於圖表的篇幅——應該像 §43.5.1 的範例那樣,在時間線的每一行後面用「⤷ 當時的資訊是……」逐一標註,讓讀者不用另外對照文字說明,就能看懂每個決定背後的判斷依據。
