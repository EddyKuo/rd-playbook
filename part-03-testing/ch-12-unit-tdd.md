...
# 第 12 章｜單元測試與 TDD 的落地
## ⸺ 測行為、不測實作,才是讓測試真正撐住你的秘訣

> **前置閱讀**:[第 11 章｜可測試的程式碼設計](./ch-11-testable-code.md)
> **下游章節**:[第 13 章｜契約測試與整合測試](./ch-13-contract-integration.md)

## 12.1 共感現場:綠燈全亮,卻還是上線出事了
...
## 12.2 真正的問題:測試測的是「實作」,不是「行為」

把小佳的情況慢慢拆開來看,會發現它其實反映了一個很常見的認知差距:我們以為測試在保護「功能」,但它實際上保護的,是某一種特定的「實作方式」。

這兩件事的差別,聽起來抽象,但結果差很遠。

當你的測試在驗「`_applyCorpRate()` 這個私有方法回傳 10」,它其實在說的是:「我依賴這個特定方法存在、有這個特定名字、必須執行這個特定邏輯。」只要你重構時改變了內部邏輯結構——例如把 `_applyCorpRate()` 和 `_applyThresholdRate()` 這兩個私有方法合併成一個 `_resolveCombinedRate()`——這支測試就失敗了,即使費率算得仍然正確。這就是為什麼測試變得脆弱:它在測實作細節,而不是在測行為的正確性——這就是「脆弱測試(Fragile Test)」的根源,它和實作細節的耦合太深,以至於重構比不寫測試還痛。
...
快的前提是:單元測試不應該碰真實資料庫、不發 HTTP request、不讀真實的設定檔。那些外部依賴會讓測試時間從毫秒跳到秒甚至分鐘。要把外部依賴隔離出去,就需要用到測試替身(Test Double)——這個家族包含 mock、stub、fake、spy 等工具,第 14 章我們會完整展開它們的選擇與寫法。這裡先記住一個原則:Fast 這個特徵,是靠「隔離外部」來達成的。
...
做法上,每支測試應該在自己的 `setUp` 裡建立自己需要的物件和狀態,跑完後的清理也由自己負責,不依賴別人留下的環境。反過來,一個共享狀態導致順序依賴的反例長這樣:

```python
# ❌ 反模式:共享狀態導致測試順序依賴
def setUp(self):
    FeeCalculator._cache = {}

def test_A():
    FeeCalculator._cache['key'] = 'value'

def test_B():
    # 這支測試只能在 test_A 之後跑才能通過
    assert FeeCalculator._cache['key'] == 'value'
```

`test_B` 看起來獨立,實際上偷偷依賴了 `test_A` 先跑過、留下的那把鑰匙。單獨執行 `test_B` 就會失敗,而你很難第一眼看出原因。
...
綠了。這一步的目標只有一個:讓測試通過。程式碼可以不漂亮,但它必須正確。在紅—綠—重構的循環裡,綠色步驟就是確認邏輯對了;程式碼的整理與美化,是後面重構步驟的事。這樣分工清楚,才能真正體會到重構時測試帶給你的安心感——如果在綠色步驟就順手把程式碼寫得漂漂亮亮,反而會混淆「這一步在確認什麼」,也會讓你少了一次「安心重構」的練習機會。
...
### 12.3.3 TDD 的務實用法:什麼時候先測、什麼時候後測
...
| 整合流程、跨服務呼叫 | **整合測試層處理(見第 13 章)** | 單元測試無法完整覆蓋這個層次 |
...
先列清單再寫測試,你就能在動手寫程式之前先問自己:「我有沒有漏掉什麼組合?」這樣做的好處是,它讓「測試覆蓋場景」而不是「測試覆蓋行數」。為什麼重要?因為你可以執行到一行程式碼,卻永遠不會觸發它的某個分支——特別是組合型條件(像費率疊加)。行數覆蓋率 80% 不代表邏輯的 80 種組合都被驗證過。但如果你先列清單再寫測試,每個業務場景組合都有一支對應的測試,失誤的餘地就小得多。
...
當你跳過 Refactor 那一步,你的程式碼會在「功能正確」的表象下慢慢腐爛:方法越來越長,命名越來越隨意,下一個接手的人越來越難讀懂。而且令人遺憾的是,這時候你有測試,你本來可以放心地整理,卻因為「趕著做下一件事」而選擇不整理。
...
### 12.5.1 範例:ClearFund 費率模組的行為場景測試卡

回到小佳的那個費率計算模組。如果在開始寫測試之前,她先填了這張卡,那個「企業戶疊加費率算錯」的 bug 很可能在測試設計階段就被發現了——它就藏在「正常路徑場景」裡一個從來沒被列出來的組合裡。

在看這張填好的卡之前,先說明每一欄實際在幫你檢查什麼,這樣你自己填的時候才知道每一行是為了擋住哪種漏洞:

> **正常路徑場景**:這一欄最容易漏掉的,是「規則疊加」的情境。每個獨立業務條件的組合都應該有一行,不能只測「最常見的那一條」。
>
> **邊界條件場景**:邊界是程式最容易「差一點」的地方。門檻值「剛好等於」和「剛好超過」的行為常常是兩回事,兩種都要各自寫一行,不能只測其中一種就當作測過門檻了。
>
> **測試健康度自查(FIBS)**:這四項是「脆弱測試」的防火牆。不自查就容易在不知不覺中寫出「耦合私有方法」的測試,等到下次重構,測試就一片紅。
...
- **前一章**:[第 11 章｜可測試的程式碼設計](./ch-11-testable-code.md) ⸺ 可測試的設計是這一章的地基;設計不可測,TDD 寸步難行
- **下一章**:[第 13 章｜契約測試與整合測試](./ch-13-contract-integration.md) ⸺ 單元測試守住行為,整合測試守住協作;兩者互補
- **強連結**:[第 14 章｜測試替身(mock/stub/fake)的取捨](./ch-14-test-doubles.md) ⸺ 本章說「用 fake/stub 隔離外部依賴」,第 14 章教你怎麼選和怎麼寫每種類型
- **強連結**:[第 9 章｜重構的時機與安全網](../part-02-craft/ch-09-refactoring.md) ⸺ 測試是重構的安全網;本章的「重構」步驟與第 9 章深度連動
