...
> **前置閱讀**:[第 11 章｜單元測試與 TDD 的落地](./ch-11-unit-tdd.md)、[第 12 章｜契約測試與整合測試](./ch-12-contract-integration.md)
> **下游章節**:[第 14 章｜測試資料與測試環境](./ch-14-test-data.md)、[第 15 章｜與 CI 整合的測試流水線](./ch-15-ci-testing.md)
...
Stub 的優點是速度快、設定簡單。缺點是它的「世界觀」完全由你決定——如果你設定的回傳值和真實 API 脫節,你就在測試一個你自己想像的世界。
...
> 一個好記的原則:如果你的 mock/stub 反映的是**你不擁有**的介面,那你需要一個機制(例如契約測試,見第 12 章)來保證它不過時。
...
這個選擇和 ClearPay 的小雅幾乎是同樣的結構——外部的用 Mock 配契約測試,有狀態的用 Fake,純副作用的用 Spy。這不是巧合:這三種依賴性質是跨領域普遍存在的模式。當你把「外部/內部」和「要驗什麼」這兩個維度想清楚,答案幾乎就出來了。
...
> 修正方向:任何你不擁有的外部介面,mock 完之後,要配套一個「契約測試(Contract Testing)」或「API 整合 smoke test」,定期驗證 mock 的假設還在。第 12 章談的契約測試,就是為了解決這個問題。

---

把五個絆倒點都走過一遍之後,你可能會想:有沒有一個工具,可以讓我在寫測試的時候、在 code review 的時候,一次把這些風險都想清楚、寫下來?答案是有的——那就是下面這張一頁式的選用卡。

## 13.5 帶得走的工具 ⸺ 一頁式「測試替身選用卡」
...
### 13.5.1 範例:ClearPay 交易授權服務的替身選用
...
【每個依賴的替身選擇】
  ...
  依賴 1:RiskEvaluationClient
    ...
    - 保護機制:契約測試(ch-12 的 Pact 契約),每次 CI 觸發驗簽
...
下一章,我們會繼續這個主題的下一個問題:要讓測試真的能跑、能重複跑,還需要想清楚測試資料從哪裡來、測試環境如何隔離——那就是第 14 章要聊的事情。

## Cross-References

- **前章**:[第 12 章｜契約測試與整合測試](./ch-12-contract-integration.md) ⸺ 本章提到的「保護 Mock 不漂移」的契約測試機制在這裡
- **下一章**:[第 14 章｜測試資料與測試環境](./ch-14-test-data.md) ⸺ 替身選好了,測試資料從哪來?
- **強連結**:[第 11 章｜單元測試與 TDD 的落地](./ch-11-unit-tdd.md) ⸺ 替身在 TDD 裡的使用時機
- **強連結**:[第 15 章｜與 CI 整合的測試流水線](./ch-15-ci-testing.md) ⸺ 契約測試如何在 CI 中定期觸發
- **強連結**:[第 37 章｜審查 AI 生成的程式碼](../part-08-ai-era/ch-37-reviewing-ai-code.md) ⸺ AI 生成的測試往往傾向全用 Mock,需要用本章的視角審查
