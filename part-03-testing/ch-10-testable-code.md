...
> **修正方向**:如果你不習慣 TDD,就先把「讓程式碼容易被測試」當成設計目標。只要設計對了,什麼時候補測試都行。第 11 章會再聊 TDD 的落地節奏。

## 10.5 帶得走的工具 ⸺ 一頁式「可測試性設計審查卡」
...
下一章,我們會繼續往前走:有了可測試的設計之後,如何讓測試本身也寫得乾淨、可維護——那是第 11 章「單元測試與 TDD 的落地」要聊的事。
