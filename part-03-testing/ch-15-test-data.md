...
> **前置閱讀**:[第 14 章｜測試替身(mock/stub/fake)的取捨](./ch-14-test-doubles.md)
> **下游章節**:[第 16 章｜與 CI 整合的測試流水線](./ch-16-ci-testing.md)

## 15.1 共感現場:那個「只有本機才能跑過」的測試
...
## 15.2 真正的問題:測試的可信度,建立在環境的可重複性上

把小燕那個案例慢慢拆開,你會發現問題不在測試邏輯寫錯了,而在一個更深的假設沒有被滿足。

測試之所以有意義,是因為我們相信**兩次執行相同的測試,應該要得到相同的結果**。這聽起來理所當然,但實際上它需要一個條件做保證:測試執行的起始狀態必須是已知的、固定的。如果起始狀態每次都不一樣——資料庫裡殘留著前一輪的資料、環境變數因機器而異、隨機種子每次不同——那測試的「結果」就不是在驗證程式邏輯,而是在驗證「今天的環境剛好長什麼樣」。
...
大多數人寫測試時只守住第一根柱子,卻讓第二根柱子靠碰運氣。本機剛好乾淨,就通過了;CI 剛好有殘留,就失敗了。而你永遠不知道失敗是因為程式碼真的有問題,還是因為環境恰好不乾淨。

更麻煩的是,當測試的可信度不夠時,有時候「通過」反而更危險——你以為沒問題,但可能只是今天環境剛好補上了你沒有測到的漏洞,下次環境換了就破了。一個測試如果不能讓你相信「綠燈代表真的沒問題」,那你的信心其實建立在不穩定的基礎上——看似牢固,其實隨時可能倒塌。
...
對大多數測試場景來說,相比直接寫死資料,使用資料建造者(Data Builder)能讓 schema 改動只需修改一處——正因為這樣的長期收益,它值得養成習慣。我們來看為什麼。它的概念很簡單:為每一種核心資料型別,寫一個會回傳「合理預設值」的建造器,然後在每個測試裡只覆蓋「這個測試真正在乎的欄位」。
...
前面談的 Builder,不管怎麼組合欄位,產生出來的都是「確定的值」——你寫死了 `stock: 0`,它就永遠是 0。但測試資料的來源還有另一個維度值得考慮:資料本身要「固定」還是「隨機」?有些讀者可能會想:「測試資料可不可以用隨機值?」
...
### 15.3.3 測試環境一致性:讓「起點」每次都一樣

理解了這套思路之後,讓我們回到小燕那次 CI 失敗的根本原因——她的測試在開始之前,沒有主動把環境帶回到預期的起點。
...
這個情況很常見。一開始放了 10 筆 seed 資料,很快就長到 100 筆——因為每次有測試需要一個稍微不同的情境,就往 fixture 裡加一筆。到最後沒有人敢刪任何一筆,因為不知道誰在用。fixture 變得像一個黑盒子,只能加不能刪,最後成為整個測試套件最脆弱的地方。
...
### 15.5.1 範例:Cartly 商品評論功能的測試環境設定
...
- **上一章**:[第 14 章｜測試替身(mock/stub/fake)的取捨](./ch-14-test-doubles.md) ⸺ 替身處理「外部依賴的隔離」,本章處理「測試環境的內部一致性」,兩者互補
- **下一章**:[第 16 章｜與 CI 整合的測試流水線](./ch-16-ci-testing.md) ⸺ 本章打好的環境隔離基礎,讓 CI 流水線能放心做平行化與快取
- **強連結**:[第 12 章｜單元測試與 TDD 的落地](./ch-12-unit-tdd.md) ⸺ Builder 模式和固定種子在單元測試同樣適用
- **強連結**:[第 13 章｜契約測試與整合測試](./ch-13-contract-integration.md) ⸺ 整合測試對環境一致性的需求最高,本章策略直接適用
