...
# 第 16 章｜與 CI 整合的測試流水線
## ⸺ 讓每一次 push,都能安心等待那道綠燈

> **前置閱讀**:[第 15 章｜測試資料與測試環境](./ch-15-test-data.md)
> **下游章節**:[第 17 章｜Code Review:看什麼、怎麼給回饋](../part-04-collaboration/ch-17-code-review.md)

## 16.1 共感現場:那條讓大家都不敢動的 CI
...
問題是,工程師不會傻傻地坐著等——他會下意識地趁著這段時間處理積壓的工作。這不是選擇,而是人的正常適應:與其盯著一個二十分鐘後才會有結果的進度條,不如先把手上另一件積壓的事往前推一點。這不只是「浪費等待時間」的問題——情境切換(context switching)本身有認知成本。工程師切去做別的事,等回來的時候,需要時間重新把思路拉回原本的問題。更關鍵的是,越慢的回饋,工程師越難把「紅燈」和「我剛才那行程式碼」連結起來。程式碼記憶是熱的,剛寫完的二十分鐘之內,你對每一行的理由都很清楚;等二十分鐘之後回來看紅燈,那份清晰度已經降溫了。
...
改一個折扣計算的邏輯,最需要立刻知道的,是單元測試有沒有過——因為這個函式的邏輯對不對,沒有比單元測試更直接的驗證方式。整合測試和 E2E 只能回答「系統有沒有跑通」,回答不了「這行程式碼的邏輯對不對」——所以不需要等 E2E 才知道自己有沒有寫錯。讓工程師等 E2E 才拿到這個答案,是一種設計上的浪費。
...
### 16.3.3 紅燈即停:讓失敗變成一個有重量的訊號

這是三個原則裡最簡單,卻最容易被忽略的一條:當 CI 跑紅了,團隊的預設行為應該是**停下來修它**,而不是先繞過去。這條原則有個常用的說法,在這裡值得先澄清一下:我們說的不是傳統開發裡的「盡早發現錯誤」(Fail Fast),而是「發現故障之後,立刻反應並中止,而不是繼續往前跑」。它的意義不只是技術設定——它更是一種團隊文化的校準。
...
| 整條 CI 長期很慢 | 回頭看 §16.3.1 和 §16.3.2 | 慢本身是一種信任問題,間接原因和 flaky 相同 |
...
就像道路會隨著年月老化,你最初設計好的流水線也會被業務增長慢慢拖垮。你不一定每天在修它,但如果三年沒有看它一眼,它已經慢了一倍你還不知道。
...
這個情況出現的原因通常有幾種:環境變數只在本機有、測試之間有順序相依、用了本機才有的服務或絕對路徑。對工程師來說,最讓人煩躁的是——當本機過、CI 紅的時候,你無法確定「是我的邏輯錯了」還是「只是環境問題」。

這個不確定性本身會侵蝕信任:因為紅燈的涵義變得模糊,工程師就開始問「這個紅燈是真的還是假的?」——而一旦這個問題出現了,你就已經在損耗整條 CI 的可信度了,這正是 §16.3.3 談過的那種信任破裂,只是這次的起點換成了環境不一致。
...
- **前一章**:[第 15 章｜測試資料與測試環境](./ch-15-test-data.md) ⸺ 流水線裡的整合測試需要可靠的測試資料與環境,這兩件事相互依存
- **下一章**:[第 17 章｜Code Review:看什麼、怎麼給回饋](../part-04-collaboration/ch-17-code-review.md) ⸺ CI 全綠是 PR 進入 review 的入場條件,兩者串聯形成完整的品質關卡
- **強連結**:[第 21 章｜CI/CD 流水線設計](../part-05-delivery/ch-21-cicd.md) ⸺ 本章談測試層的設計與分層,第 21 章談整條交付流水線的架構
- **強連結**:[第 11 章｜可測試的程式碼設計](./ch-11-testable-code.md) ⸺ 可測試的設計是流水線能快的前提;模組邊界清楚,才能有效按模組平行化
