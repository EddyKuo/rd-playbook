...
part: III
title: 契約測試與整合測試
slug: ch-12-contract-integration
agent: RD
...
> **前置閱讀**:[第 11 章｜單元測試與 TDD 的落地](./ch-11-unit-testing-tdd.md)
...
> **修正方向**:契約只描述消費者實際用到的欄位,不用到的不管它。Pact 預設的比對行為叫做「寬鬆比對(Loose matching)」——提供者多給沒關係,只要消費者要的欄位在、型別對就好。把這個原則說清楚,契約會安靜很多。
...
- **上一章**:[第 11 章｜單元測試與 TDD 的落地](./ch-11-unit-testing-tdd.md) ⸺ 單元測試守業務邏輯,本章守服務邊界
- **下一章**:[第 13 章｜測試替身(mock/stub/fake)的取捨](./ch-13-test-doubles.md) ⸺ 契約測試裡用到的 stub/mock 選型深探
- **強連結**:[第 15 章｜與 CI 整合的測試流水線](./ch-15-ci-test-pipeline.md) ⸺ 把契約測試接入 CI 的具體做法
